...
// Canonicalize the denylist itself: on macOS realpath resolves /etc -> /private/etc,
// /var -> /private/var, etc. Comparing a realpath'd target against a raw /etc prefix
// would MISS /etc/passwd. Cache both the raw and realpath'd prefixes once.
let _canonicalPrefixes: string[] | null = null;
async function canonicalSystemPrefixes(): Promise<string[]> {
  if (_canonicalPrefixes) return _canonicalPrefixes;
  const set = new Set<string>();
  for (const p of SENSITIVE_SYSTEM_PREFIXES) {
    set.add(p);
    try {
      set.add(await realpath(p));
    } catch {
      // prefix may not exist on this platform
    }
  }
  _canonicalPrefixes = [...set];
  return _canonicalPrefixes;
}

async function isSensitivePath(canonical: string): Promise<string | null> {
  for (const prefix of await canonicalSystemPrefixes()) {
  ...
  try {
    ...
    // keep raw home if it cannot be resolved
  }
...
export async function assertSafeLocalPath(input: string): Promise<string> {
  ...
  const hit = await isSensitivePath(canonical);
  if (hit) {

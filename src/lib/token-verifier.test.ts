...
describe("createTokenVerifier", () => {
  ...
  describe("JWT verification", () => {
    ...
    it('defaults scopes to ["mcp:read"] when mcp_scopes is not an array', async () => {
      mockJwtVerify.mockResolvedValue(
        jwtPayload({ app_metadata: { mcp_scopes: "not-an-array" } }),
      );
    ...
    it("uses client_id from JWT when present", async () => {
      mockJwtVerify.mockResolvedValue(
        jwtPayload({ client_id: "my-oauth-client" }),
      );
  ...
  describe("API key verification", () => {
    ...
    it("returns AuthInfo for a valid API key with all fields", async () => {
      ...
      const expectedExp = Math.floor(
        new Date("2027-06-15T00:00:00Z").getTime() / 1000,
      );
    ...
    it('throws "API key expired" when expiresAt is in the past', async () => {
      ...
      await expect(
        verifier.verifyAccessToken("snk_live_expired"),
    ...
    it("throws when API key validation returns valid: false", async () => {
      ...
      await expect(
        verifier.verifyAccessToken("snk_live_revoked"),
      ).rejects.toThrow("Key has been revoked");
    });
    ...
    it('throws generic "Invalid API key" when valid: false without error message', async () => {
      ...
      await expect(
        verifier.verifyAccessToken("snk_live_invalid"),
    ...
    it("throws when validation returns valid: true but no userId", async () => {
      ...
      await expect(
        verifier.verifyAccessToken("snk_live_nouser"),
      ).rejects.toThrow("Invalid API key");
    });
    ...
    it("throws on HTTP 401 from Edge Function", async () => {
      ...
      await expect(
        verifier.verifyAccessToken("snk_live_unauth"),
      ).rejects.toThrow("API key validation failed: HTTP 401");
    });
    ...
    it("throws on HTTP 500 from Edge Function", async () => {
      ...
      await expect(
        verifier.verifyAccessToken("snk_live_servererr"),
      ).rejects.toThrow("API key validation failed: HTTP 500");
    });
    ...
    it("throws timeout error when fetch is aborted", async () => {
      ...
      await expect(
        verifier.verifyAccessToken("snk_live_timeout"),
      ).rejects.toThrow("API key validation timed out");
    });
    ...
    it("propagates network errors from fetch", async () => {
      ...
      await expect(
        verifier.verifyAccessToken("snk_live_network"),
  ...
  describe("JWKS caching", () => {
    it("creates JWKS only once across multiple JWT verifications", async () => {
      ...
      const keysets = mockJwtVerify.mock.calls.map(
        (call: unknown[]) => call[1],
      );

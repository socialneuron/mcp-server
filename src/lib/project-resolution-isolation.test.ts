  ...
  createClient: vi.fn(() => ({
    from: vi.fn((table: string) => {
      if (table === 'organization_members') {
        const query = {
          select: vi.fn(() => query),
          eq: vi.fn(async () => ({ data: [{ organization_id: 'org-a' }], error: null })),
        };
        return query;
      }
      if (table === 'projects') {
        ...
        const query = {
          ...
          in: vi.fn(() => query),
          order: vi.fn(() => ({
            then: (resolve: (v: { data: typeof accessibleProjects; error: null }) => void) =>
      ...
      if (table === 'connected_accounts') {
        const query = {
          ...
          eq: vi.fn((_col: string, value: string) => {
            ...
            return {
              in: vi.fn(async () => ({
                data: connectedAccountRows.filter(r => (r.user_id ?? 'user-a') === value),
                error: null,
              })),
            };
          }),
...
describe('MCP default project isolation', () => {
  ...
  describe('resolveProjectForConnectedAccountTool', () => {
    it('passes an explicit project_id straight through without any DB call', async () => {
      const { resolveProjectForConnectedAccountTool } = await import('./supabase.js');

      const result = await resolveProjectForConnectedAccountTool('explicit-project');
    ...
    it('treats expires_soon as active for the sole-account-owner auto-resolve', async () => {
      ...
      connectedAccountRows.push({ project_id: 'project-a', status: 'expires_soon' });
      const { resolveProjectForConnectedAccountTool } = await import('./supabase.js');
    ...
    it("a teammate's account does not break YOUR OWN sole-candidate resolution either", async () => {
      ...
      connectedAccountRows.push(
        { project_id: 'project-a', status: 'active', platform: 'instagram', user_id: 'user-a' },
        { project_id: 'project-b', status: 'active', platform: 'tiktok', user_id: 'user-b' }
      );
      const { resolveProjectForConnectedAccountTool } = await import('./supabase.js');
    ...
    it('is platform-aware: a project with only an UNRELATED platform account does not count', async () => {
      ...
      connectedAccountRows.push({ project_id: 'project-a', status: 'active', platform: 'youtube' });
      const { resolveProjectForConnectedAccountTool } = await import('./supabase.js');

      const result = await resolveProjectForConnectedAccountTool(undefined, 'instagram');

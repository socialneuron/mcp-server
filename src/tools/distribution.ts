 ...
 *
 * The audit was approved around 2026-05-30. Operators can set
...
const VIDEO_FILE_EXTENSIONS = new Set(['mp4', 'mov', 'm4v', 'webm', 'avi', 'mkv', 'mpeg', 'mpg']);
const IMAGE_FILE_EXTENSIONS = new Set(['jpg', 'jpeg', 'png', 'webp', 'gif', 'avif', 'heic']);
...
export function registerDistributionTools(server: McpServer): void {
    ...
    async ({
      ...
      const signR2Key = async (key: string): Promise<string | null> => {
        ...
        const { data: signData } = await callEdgeFunction<{
          signedUrl?: string;
          url?: string;
        }>('get-signed-url', { r2Key: cleanKey, operation: 'get' }, { timeoutMs: 10_000 });
      ...
      try {
        ...
        if (shouldRehost && resolvedMediaUrl && !resolvedMediaUrlIsTrustedR2) {
          const rehost = await rehostExternalUrl(resolvedMediaUrl, resolvedProjectId);
          if ('error' in rehost) {
            return {
              content: [
                {
      ...
      // MCP is an AI-assisted publishing surface, so use each platform's
      // native AI disclosure by default. An explicit false remains available
      // for verified human-shot/non-AI media.
      ...
      const { data, error } = await callEdgeFunction<SchedulePostResult>(
        'schedule-post',
        {
          ...
          ...(normalizedPlatformMetadata
            ...
            : {}),
          // P1-8 (2026-07-15 review): ALWAYS send an idempotency key. A 30s
          // client timeout followed by an agent retry used to mint a second
          // live publish; a stable key derived from the logical request makes
          // retries collapse server-side (the EF already supports it).
          idempotencyKey:
            idempotency_key ??
            createHash('sha256')
              .update(
                JSON.stringify({
                  projectId: resolvedProjectId,
                  // Sorted: platform order must not vary the retry key (#2304).
                  platforms: [...normalizedPlatforms].sort(),
                  caption: finalCaption ?? '',
                  title: title ?? '',
                  media: media_urls ?? media_url ?? r2_keys ?? r2_key ?? null,
      ...
      if (error) {
        const timeoutHint = /tim(ed)? ?out/i.test(String(error))
          ? ' The request may still have completed server-side — check list_recent_posts before retrying. Retrying with identical inputs is safe: the idempotency key collapses duplicates.'
          : '';
        return {
          content: [
            {
              type: 'text' as const,
              text: `Failed to schedule post: ${error}${timeoutHint}`,
            },
          ],
          isError: true,
        };
  ...
  server.tool(
    ...
    async ({ post_id, project_id, scheduled_at, expected_scheduled_at, response_format }) => {
  ...
  server.tool(
    ...
    async ({ project_id, include_all, response_format }) => {
      ...
      if (parsedAccounts.some(account => account.project_id !== resolvedProjectId)) {
        return {
          content: [
            {
              type: 'text' as const,
  ...
  server.tool(
    ...
    async ({ project_id, platform, status, days, limit, response_format }) => {
      ...
      // user's projects so agents can retry with an explicit project_id.
  ...
  server.tool(
    ...
    async ({ project_id, platforms, count, start_after, min_gap_hours, response_format }) => {
      try {
        const resolvedProjectId = project_id ?? (await getDefaultProjectId()) ?? undefined;
        if (!resolvedProjectId) {
          return {
        ...
        if (!Number.isFinite(startDate.getTime())) {
          return {
            content: [{ type: 'text' as const, text: 'start_after must be a valid ISO datetime.' }],
            isError: true,
          };
        }
  ...
  server.tool(
    ...
    'Schedule all posts in a content plan. Optionally auto-assigns time slots and runs quality checks before scheduling. Supports dry-run mode.',
    {
      ...
      account_ids: z
        .record(z.string(), z.string().uuid())
        .optional()
        .describe('Exact connected-account ID per platform for every post in the plan.'),
      plan_id: z
        .string()
        .uuid()
        .optional()
    ...
    async ({
      ...
      try {
        ...
        if (effectivePlanId) {
          ...
          // Fail CLOSED (2026-07-15 review P1-7): a non-success response or a
          // renamed/missing items field used to collapse to [] and skip the
          // approval gate entirely — every post scheduled with no approval.
          if (approvalsResult?.success !== true || !Array.isArray(approvalsResult.items)) {
            return {
              content: [
                {
                  type: 'text' as const,
                  text:
                    `Could not verify plan approvals for ${effectivePlanId} — the approvals ` +
                    'lookup returned an unexpected response. Refusing to schedule unverified ' +
                    'posts; retry, or review the plan in the app.',
                },
              ],
              isError: true,
            };
        ...
        if (effectiveProjectId) {
          // Fail CLOSED (2026-07-15 review P1-7): a swallowed transport error or
          // non-success response used to silently fall back to default safety
          // config (threshold 26, no banned terms, no brand avoid patterns) —
          // the brand's own guardrails vanished exactly when the platform
          // hiccuped. A brand with no custom settings still returns
          // success:true with defaults; only a real failure blocks.
          const { data: safetyResult, error: safetyError } = await callEdgeFunction<{
            success: boolean;
          ...
          if (safetyError || safetyResult?.success !== true) {
            return {
              content: [
                {
                  type: 'text' as const,
                  text:
                    "Could not load this brand's content-safety settings " +
                    `(${safetyError || 'unexpected response'}). Refusing to schedule with ` +
                    'default safety config — retry in a moment.',
                },
              ],
              isError: true,
            };
        ...
        for (const [platform, accountId] of Object.entries(account_ids ?? {})) {
          if (embeddedAccountIds[platform] && embeddedAccountIds[platform] !== accountId) {
            return {
              content: [
                {
                  type: 'text' as const,
        ...
        const planPlatforms = Array.from(new Set(workingPlan.posts.map(post => post.platform)));
        ...
        const results: Array<{
          ...
          error?: string;
          error_type?:
            'quality' | 'safety' | 'platform_policy' | 'rate_limit' | 'transport' | 'unknown';
          retryable?: boolean;
        }> = [];
        ...
        const scheduleOne = async (
          ...
          const { data, error } = await callEdgeFunction<SchedulePostResult>(
            'schedule-post',
            {
              ...
              ...(effectiveProjectId
                ? { projectId: effectiveProjectId, project_id: effectiveProjectId }
                : {}),
              connectedAccountIds: {

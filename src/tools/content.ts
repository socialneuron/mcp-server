...
function buildFallbackDisclosureLine(meta: AsyncJob['result_metadata']): string | null {
  ...
  const reason = meta?.fallback_reason ? ` (upstream failure: ${meta.fallback_reason})` : '';
    ...
    async ({
      ...
      const { data, error } = await callEdgeFunction<GenerateVideoResponse>(
        'kie-video-generate',
        {
          ...
          // Default FALSE (2026-07-13) for kling-family etc: the old `?? true` default
          // silently multiplied cost (2.65x on kling 2.6) for callers that never asked
          // for audio. seedance-2/-fast are native audio-video models with no audio
          // cost multiplier (kie-video-generate/index.ts `generate_audio = enableAudio
          // !== false`, defaults true) — this tool must not override that default to a
          // silent `false`, or every MCP seedance call ships mute unless the caller
          // remembers to pass `enable_audio: true` (2026-07-20 pilot silent-video bug).
          enableAudio: enable_audio ?? (model === 'seedance-2' || model === 'seedance-2-fast'),
  ...
  server.tool(
    ...
    'Start an async AI image generation job — returns a job_id immediately. Poll with check_status every 10-15s until complete. Costs 15-50 credits depending on model. Use for social media posts, carousel slides, or as input to generate_video (image-to-video). Pass project_id so the asset is stored with the correct brand/project.',
    {
      ...
      image_url: z
        ...
        .optional()
        .describe(
          // 2026-07-16: "Required for ideogram model" removed — ideogram is not in the
          // accepted `model` enum above, so that claim was never true for this tool.
          'Reference image URL for image-to-image generation. Optional for all currently ' +
            'accepted models (none of them require it).'
        ),
      project_id: z
        .string()
        .optional()
  ...
  server.tool(
    ...
    async ({ job_id, response_format }) => {
      ...
      let projectsDisclosure:
        Awaited<ReturnType<typeof listAccessibleProjectsWithAccountStatus>> | undefined;
...
Return ONLY valid JSON in this exact format:
{
  ...
  "frames": [
    {
      ...
      // config.responseMimeType marks this as STRUCTURED output on the EF side.
      // Without it the EF treats the storyboard as prose: it prepends the
      // anti-slop caption-writing rules to the prompt AND runs the post-gen
      // slop gate over the storyboard JSON — cinematography prompt language
      // trips the gate and the whole call 422s (found live 2026-07-15).
      ...
      // generation is never discarded as an empty MCP response. Strip markdown
      // fences defensively: Gemini occasionally wraps JSON in ```json blocks.
  ...
  server.tool(
    'generate_voiceover',
    'Generate a voiceover audio file for video narration. Returns an R2-hosted audio URL. Use after create_storyboard to add narration to each scene, or standalone for podcast intros and ad reads. Pass project_id to keep the asset with the correct brand/project. Costs 15 credits per generation. If a prior call returned "still generating", pass its task_id back as resume_task_id to check again WITHOUT a second charge — never call this fresh to "retry" a still-generating job.',
    {
      text: z
        .string()
        .max(5000)
        .optional()
        .describe('The script/text to convert to speech. Not required when resume_task_id is set.'),
      voice: z
        // 2026-07-15: generate_voiceover routes through kie.ai's
        // elevenlabs/text-to-speech-turbo-2-5 model (kie-tts-generate),
        // which takes a capitalized voice NAME string, not an ElevenLabs
        // voiceId. Every enum value here MUST have a matching entry in
        // KIE_TTS_VOICE_NAMES below. 'domi' was previously advertised with no
        // backing allow-listed ID (100% call failure) — never re-add it
        // without first verifying the name against kie.ai's model.
        .enum(['rachel', 'daniel', 'adam', 'gigi', 'charlotte'])
        .optional()
        .describe(
          'Voice selection. rachel=warm female (default), daniel=British authoritative male, ' +
            'adam=deep confident male, gigi=young playful female, charlotte=sophisticated elegant female.'
        ),
      speed: z
        .number()
        .min(0.5)
      ...
      resume_task_id: z
        ...
        .optional()
        .describe(
          '2026-07-15 (money-safety fix): pass the task_id from a prior "still generating" ' +
            'response to resume polling the SAME job — this does NOT create a new task and ' +
            'does NOT charge credits again. Use this instead of calling the tool fresh after ' +
            'a timeout: the original job is already charged and still running or refundable; ' +
            'a fresh call would charge a SECOND time for work already paid for.'
        ),
    },
    async ({ text, voice, speed, project_id, response_format, resume_task_id }) => {
      const format = response_format ?? 'text';
      const userId = await getDefaultUserId();
      ...
      if (resume_task_id) {
        ...
        // entirely. No new charge, no new kie-tts-generate call.
        ...
        if (!text) {
          return {
            content: [
              {
                type: 'text' as const,
                text: 'text is required unless resuming a prior job via resume_task_id.',
              },
            ],
            isError: true,
          };
        ...
        const rateLimit = checkRateLimit('generation', `generate_voiceover:${userId}`);
        if (!rateLimit.allowed) {
          return {
            content: [
              {
        ...
        // 2026-07-15: re-routed off the official ElevenLabs API (`elevenlabs-tts`,
        // requires an ELEVENLABS_API_KEY SN does not hold) onto the kie.ai
        // aggregator pair (`kie-tts-generate` + `kie-tts-status`), matching every
        // other MCP generation tool (image/video/music all route through kie.ai
        // with KIE_API_KEY).
        //
        // kie.ai's `elevenlabs/text-to-speech-turbo-2-5` model takes a `voice`
        // NAME string (verified from kie-tts-generate's live source, default
        // 'Rachel') — capitalized display names, not raw ElevenLabs voiceId
        // hashes. Keep this map's keys aligned with the tool's `voice` enum above.
        const KIE_TTS_VOICE_NAMES: Record<string, string> = {
          rachel: 'Rachel',
          daniel: 'Daniel',
          adam: 'Adam',
          gigi: 'Gigi',
        ...
        const { data: createData, error: createError } = await callEdgeFunction<{
        ...
        if (createError) {
          return {
            content: [
              {
                type: 'text' as const,
                text: `Voiceover generation failed: ${createError}`,
              },
            ],
            isError: true,
          };
        ...
        // kie-tts-generate refunds synchronously (CreditHandle.refund) on both
        // of these failure paths before returning — safe to report as terminal,
        // no charge is left outstanding.
        if (createData?.status === 'failed' || !createData?.taskId) {
          return {
            content: [
              {
                type: 'text' as const,
                text: `Voiceover generation failed: ${createData?.error ?? 'no task started.'}`,
              },
            ],
            isError: true,
          };
      ...
      // Inline-poll kie-tts-status. TTS jobs are short (~11-24s observed in
      // mcp_audit_log for equivalent-length scripts) so this preserves the
      // tool's synchronous `audioUrl` contract rather than switching to the
      // job_id + check_status async pattern used by generate_video/generate_image.
      //
      // 2026-07-15 (money-safety fix, independent review): credits are debited
      // server-side in kie-tts-generate BEFORE this loop starts. If this loop
      // gives up — on a transport-level error calling kie-tts-status, or on
      // the 45s timeout — the job is still real, still running (or refundable),
      // and NOTHING else re-polls it on the MCP side. Reporting that as a
      ...
      // credits_cost > 0). So an abandoned kie-tts job is refunded within an
      // hour even if nobody ever calls kie-tts-status again. A genuinely
      // TERMINAL status from kie-tts-status itself (status==='failed') is
      // still safe to report as a hard failure here — kie-tts-status's own
      // HIGH-3 fix already fired safe_refund_credits before returning that.
      ...
      while (Date.now() < pollDeadline) {
        ...
        const { data: statusData, error: statusErr } = await callEdgeFunction<{
          status: 'processing' | 'completed' | 'failed';
          resultUrl?: string;
          durationSeconds?: number;
          error?: string;
        }>('kie-tts-status', { taskId }, { timeoutMs: 15_000 });

        if (statusErr) {
          // Transport-level failure calling kie-tts-status itself (network,
          // gateway 5xx, timeout) — transient. Retry within the SAME poll
          // budget instead of giving up on the first hiccup.
          consecutiveTransientErrors += 1;
          if (consecutiveTransientErrors >= MAX_CONSECUTIVE_TRANSIENT_ERRORS) {
            // Exhausted retries — fall through to the "still pending" report
            // below. NEVER terminal: the job itself is untouched.
            break;
          }
          continue;
        }
        ...
        if (statusData?.status === 'completed') {
          audioUrl = statusData.resultUrl ?? undefined;
          durationSeconds = statusData.durationSeconds;
          break;
        }

        if (statusData?.status === 'failed') {
          // Genuine terminal failure — kie-tts-status already refunded
          // synchronously before returning this. Safe to report as terminal.
          terminalError = statusData.error ?? 'Voiceover task failed.';
          break;
        }
      ...
      if (terminalError) {
        return {
          content: [
            {
              type: 'text' as const,
              text: `Voiceover generation failed: ${terminalError} (credits already refunded — safe to try again with a new call).`,
            },
          ],
          isError: true,
        };
      ...
      if (!audioUrl) {
        // Not terminal — still processing, or the poll gave up on transient
        // errors, or timed out. The job is real and charged; never advise a
        // fresh (re-charging) call.
        const pendingText = [
          `Voiceover still generating (job ${taskId}).`,
          `You have already been charged 15 credits for this job — do NOT call generate_voiceover again with the same text, that would charge a SECOND time.`,
          `Instead, call generate_voiceover again with resume_task_id="${taskId}" in about a minute to check again (no additional charge).`,
          `If the job never completes, credits are automatically refunded within about an hour (hourly stale-job cleanup).`,
        ].join('\n');
        ...
        return {
          content: [{ type: 'text' as const, text: pendingText }],
        };
      }

      const data = { audioUrl, durationSeconds };

      // Track local per-run spend at the point of CONFIRMED success, whether
      // that happened on the original call or a resume_task_id call — the
      // real server-side charge already happened exactly once in
      // kie-tts-generate regardless of how many polls it took to observe it.
      addCreditsUsed(15);
  ...
  server.tool(
    ...
    async ({
      ...
      const userId = await getDefaultUserId();
      const rateLimit = checkRateLimit('generation', `generate_carousel:${userId}`);
      if (!rateLimit.allowed) {
        return {
          content: [
            {

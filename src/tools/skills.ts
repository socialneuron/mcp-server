...
export function registerSkillsTools(server: McpServer): void {
  ...
  server.tool(
    ...
    async ({ studio, featured_only, response_format }) => {
      ...
      if (!efError && Array.isArray(catalogRows) && catalogRows.length > 0) {
        // DB rows are GUIDE skills (read via get_skill by slug); vendored
        // manifest entries are executable WORKFLOW skills (run_skill by id).
        // Return both so a healthy DB path cannot break list_skills → run_skill.
        const workflows = listSkills({ studio, featuredOnly: featured_only });
        // `studio` is a workflow-only dimension. Do not return unrelated guide
        // rows when the caller asked for one studio; for featured-only guide
        // queries, require an explicit first-party frontmatter flag.
        const filteredGuideRows = catalogRows.filter(
          (row) =>
            !studio && (!featured_only || row.frontmatter?.featured === true),
        );
        const guides = filteredGuideRows.map((row) => ({
          ...row,
          use_with: "get_skill" as const,
        }));
        ...
        const guideBlocks = filteredGuideRows
          .map(
            (row) =>
      ...
      // Release-1 fallback: vendored manifest when the Edge Function errors or
      // returns no rows. Remove when the DB catalogue becomes the sole source.
          ...
          content: [
            {
              type: "text" as const,
              text: JSON.stringify(
                asEnvelope({ count: skills.length, skills }),
                null,
                2,
              ),
            },
  ...
  server.tool(
    ...
    async ({ skill_id, topic, audience, hook, cta, response_format }) => {
      ...
      const runUrl =
        "https://socialneuron.com/dashboard/creation?skill=" +
  ...
  server.tool(
    ...
    async ({ slug, response_format }) => {
      ...
      // The private service returns locked metadata for discovery/upsell but
      // must never expose the paid guide body to a lower-tier caller.
      if (skill.locked) {
        return toolError(
          "permission_denied",
      ...
      if (format === "json") {
        return {
          content: [
            {
              type: "text" as const,
              text: JSON.stringify(asEnvelope(skill), null, 2),
            },
          ],
        };
      }
      ...
      return {
        content: [
          { type: "text" as const, text: `${header}${skill.body}${compiled}` },
        ],
      };
    },
  );

...
function resetMocksToDefaults(): void {
  vi.clearAllMocks();
  // Reset edge function mock to safe default
...
describe("Module: content", () => {
  ...
  it("check_status: returns completed job details", async () => {
    ...
    mockGetClient.mockReturnValueOnce({ from: vi.fn(() => chain) } as any);
...
describe("Module: insights", () => {
  ...
  it("get_performance_insights: returns empty when no projects for user", async () => {
...
describe("Module: ideation-context", () => {
  ...
  it("get_ideation_context: returns error when no org membership", async () => {
    // Default supabase mock returns null from single(), so tool can't find org membership
  ...
  it("get_ideation_context: happy path returns context with mocked data", async () => {
    const orgChain = chainMock({
      data: { organization_id: "org-1" },
      error: null,
    });
    // Project ID must match what getDefaultProjectId returns ('test-project-id')
    const projectsChain = chainMock({
    ...
    const result = await server.getHandler("get_ideation_context")!({});
    expect(result.content[0].text).toContain("Ideation Context");
    expect(result.isError).toBeUndefined();
  });
});

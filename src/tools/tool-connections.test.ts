...
const YOUTUBE_ACCOUNT_ID = "22222222-2222-4222-8222-222222222222";

/**
 * Queues the mcp-data connected-accounts inventory response the shared
 * routing lib fetches (comments.ts / youtube-analytics.ts, F3 2026-07-15).
 * Must be queued BEFORE the test's own mockResolvedValueOnce for the real
 * action call, since the routing preflight fetch happens first.
 */
function mockYouTubeAccountRouting(accountId = YOUTUBE_ACCOUNT_ID): void {
    ...
    async (fnName: string, body: Record<string, unknown>) => {
      if (fnName === "mcp-data") {
        ...
        if (action === "recent-posts")
          return { data: { success: true, posts: [] }, error: null };
        if (action === "job-status")
          return { data: { success: true, job: null }, error: null };
        if (action === "analytics")
...
describe("Module: content", () => {
  ...
  it("check_status: returns completed job details", async () => {
    ...
    const result = await server.getHandler("check_status")!({
      job_id: "job-1",
    });
  ...
  it("generate_voiceover: error path returns isError", async () => {
    ...
    expect(result.content[0].text).toMatch(
      /TTS quota exceeded|Voiceover generation failed/,
    );
  });
});
...
describe("Module: youtube-analytics", () => {
  ...
  it("fetch_youtube_analytics: happy path calls youtube-analytics edge fn", async () => {
    ...
    const result = await server.getHandler("fetch_youtube_analytics")!({
      ...
      end_date: "2026-02-01",
      project_id: "test-project-id",
      connected_account_id: YOUTUBE_ACCOUNT_ID,
    });
    expect(result.isError).toBeUndefined();
    expect(result.content[0].text.length).toBeGreaterThan(0);
  });
...
describe("Module: comments", () => {
  ...
  it("list_comments: happy path calls youtube-comments edge fn", async () => {
    ...
    expect(mockCallEdge.mock.calls[1][0]).toBe("youtube-comments");
  });
...
describe("Module: ideation-context", () => {
  ...
  it("get_ideation_context: returns error on EF failure", async () => {
    ...
    expect(result.isError).toBe(true);
    expect(result.content[0].text).toContain(
      "Failed to fetch ideation context",
    );
  });
});
...
describe("Module: autopilot", () => {
  ...
  it("list_autopilot_configs: returns empty list with no configs", async () => {
    ...
    mockGetClient.mockReturnValueOnce(
      supabaseWithChain({ data: [], error: null }),
    );
    ...
    mockCallEdge.mockImplementation(((
      ...
      if (fn === "scrape-youtube" && body.action === "transcript") {
        return Promise.resolve({
          data: {
            success: true,
            data: { segments: [{ text: "Hello world transcript" }] },
          },
          error: null,
        });
      }
  ...
  it("plan_content_week: happy path calls social-neuron-ai", async () => {
    ...
    mockCallEdge
      .mockResolvedValueOnce({
        data: {
          success: true,
          profile: { brand_name: "Acme", brand_context: {} },
        },
        error: null,
      })
      .mockResolvedValueOnce({

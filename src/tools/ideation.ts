...
export function registerIdeationTools(server: McpServer): void {
  ...
  server.tool(
    ...
    async ({
      ...
      if (project_id) {
        try {
          const [{ data: brandData }, { data: ideationData }] =
            await Promise.all([
              callEdgeFunction<{
                success?: boolean;
          ...
          const brandIndustry = brandContext?.industryClassification as
            | string
            | undefined;
          const voiceProfile =
            (brandContext?.voiceProfile as
              | Record<string, unknown>
              | undefined) ?? {};
          ...
          const languagePatterns =
            Array.isArray(voiceProfile.languagePatterns) &&
            voiceProfile.languagePatterns.length > 0
              ? voiceProfile.languagePatterns.map(String).join("; ")
              : "";
          const avoidPatterns =
            Array.isArray(voiceProfile.avoidPatterns) &&
            voiceProfile.avoidPatterns.length > 0
              ? voiceProfile.avoidPatterns.map(String).join(", ")
              : "";
          const sampleContent =
            typeof voiceProfile.sampleContent === "string"
          ...
          const voiceParts = [
            ...
            style ? `Style: ${style}` : "",
            languagePatterns
              ? `Use these language patterns: ${languagePatterns}`
              : "",
          ...
          if (platformOverride) {
            const platformTone =
              Array.isArray(platformOverride.tone) &&
              platformOverride.tone.length > 0
                ? platformOverride.tone.map(String).join(", ")
                : "";
            const platformStyle =
              Array.isArray(platformOverride.style) &&
              platformOverride.style.length > 0
          ...
          if (
            typeof perfInjection === "string" &&
            perfInjection.trim().length > 0
          ) {
  ...
  server.tool(
    ...
    async ({
      ...
      const platformGuidelines: Record<string, string> = {
        twitter:
          "Max 280 characters. Concise, punchy. 1-3 hashtags max. Thread-friendly.",
        threads:
          "Max 500 characters. Conversational, opinion-driven. Minimal hashtags.",
        bluesky: "Max 300 characters. Community-focused, authentic tone.",
        instagram:
          "Up to 2200 characters. Visual-first captions. 5-15 hashtags. Use line breaks.",
        tiktok:
        ...
        facebook:
          "Up to 63206 characters. Conversational. Questions engage. Share-friendly.",
      };
      ...
      if (project_id) {
        try {
          ...
          const voiceProfile = (
            brandData?.profile?.brand_context as
              | Record<string, unknown>
              | undefined

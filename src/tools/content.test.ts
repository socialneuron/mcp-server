...
function chainMock(resolvedValue = { data: null, error: null }) {
  ...
  ["select", "eq", "or", "limit", "maybeSingle", "order", "gte", "in"].forEach(
    (m) => {
...
describe("content tools", () => {
  ...
  describe("check_status", () => {
    ...
    it("polls live status via kie-task-status when job is pending with external_id", async () => {
      ...
      mockCallEdge
        .mockResolvedValueOnce({
          data: { success: true, job: pendingJob },
          error: null,
        })
        .mockResolvedValueOnce({
          data: {
    ...
    it("discloses when a different fallback model delivered the result", async () => {
      ...
      expect(result.content[0].text).toContain(
        "cost never exceeds the requested model's price",
      );
    });
  });
        ...
        data: {
          carousel: {
            ...
            slides: [
              {
                slideNumber: 1,
                headline: "STOP BEING BROKE",
                emphasisWords: ["BROKE"],
              },
              {
                slideNumber: 2,
                headline: "MONEY IS SIMPLE",
                emphasisWords: ["SIMPLE"],
              },
            ],
      ...
      mockCallEdge.mockResolvedValueOnce({
        data: {
          carousel: {
            id: "carousel_456",
            slides: [
              { slideNumber: 1, headline: "TEST", emphasisWords: ["TEST"] },
            ],
  ...
  describe("generate_voiceover", () => {
    it("maps the voice name to a real ElevenLabs voiceId and forwards speed", async () => {
      mockCallEdge.mockResolvedValueOnce({
        data: { audioUrl: "https://r2.example/audio.mp3", durationSeconds: 5 },
        error: null,
      });
      ...
      const [fn, body] = mockCallEdge.mock.calls[0] as [
        string,
        Record<string, unknown>,
      ];
      expect(fn).toBe("elevenlabs-tts");
    ...
    it("defaults to the rachel voiceId when no voice is given", async () => {
      mockCallEdge.mockResolvedValueOnce({
        data: { audioUrl: "https://r2.example/a.mp3" },
        error: null,
      });
      await server.getHandler("generate_voiceover")!({
        text: "Hi",
      ...
      expect(body.voiceId).toBe("21m00Tcm4TlvDq8ikWAM"); // rachel
    });
  });
});

...
describe('insights tools', () => {
  ...
  describe('get_performance_insights', () => {
    it('resolves user org, projects, and scopes insights to those project IDs', async () => {
      ...
      const projChain = chainMock({ data: [{ id: 'proj-1' }, { id: 'proj-2' }], error: null });
      ...
      expect(insightsChain.in).toHaveBeenCalledWith('project_id', ['proj-1', 'proj-2']);
    ...
    it('returns "No projects found" when user has no org membership', async () => {
      ...
      expect(result.content[0].text).toContain('No projects found for current user');
    ...
    it('filters by insight_type when provided', async () => {
      ...
      expect(insightsChain.eq).toHaveBeenCalledWith('insight_type', 'optimal_timing');
    });
    ...
    it('uses default 30-day lookback and respects custom days param', async () => {
      ...
      // Custom: 7 days
      vi.clearAllMocks();
      const orgChain2 = chainMock({ data: { organization_id: 'org-1' }, error: null });
  ...
  describe('get_best_posting_times', () => {
    ...
    it('returns top 5 slots sorted by avg_engagement descending', async () => {
      ...
      mockGetClient.mockReturnValue({ from: vi.fn(() => analyticsChain) } as any);

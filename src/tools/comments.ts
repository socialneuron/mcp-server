...
async function exactYouTubeRoute(
  ...
  { projectId: string; connectedAccountId: string } | { error: string }
> {
  const resolvedProjectId =
    projectId ?? (await getDefaultProjectId()) ?? undefined;
  ...
  const routing = await resolveConnectedAccountRouting({
    ...
    platforms: ["youtube"],
    requestedAccountIds: connectedAccountId
  ...
  server.tool(
    ...
    async ({
      ...
      for (const c of comments) {
        const excerpt =
          c.textOriginal.length > 120
            ? c.textOriginal.slice(0, 120) + "..."
            : c.textOriginal;
          ...
          content: [
            {
              type: "text" as const,
              text: JSON.stringify(
                asEnvelope({ success: true, commentId: comment_id }),
                null,
                2,
              ),
            },

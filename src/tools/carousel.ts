...
export function registerCarouselTools(server: McpServer): void {
  server.tool(
    ...
    'End-to-end carousel creation: generates slide text + kicks off image generation for each slide in parallel. When brand_id is provided, auto-injects brand colors, logo watermark, and visual mood into every image prompt. Returns carousel data + image job_ids. Poll each job_id with check_status until complete, then call schedule_post with job_ids to publish as Instagram carousel (media_type=CAROUSEL_ALBUM).',
    {
      ...
      template_pack_id: z
        .enum(CAROUSEL_TEMPLATE_PACK_IDS)
        .optional()
        .describe(
          'Canonical template pack id (drives slide structure + count server-side, overrides template_id when both are given). ' +
            'sn_signal_carousel_v1 = 7-slide hook/insight/framework deck (same as hormozi-authority/educational-series). ' +
            'sn_performance_recap_v1 = 5-slide before/after metric recap. ' +
            'sn_showcase_ad_v1 = 6-slide product showcase ad.'
        ),
      slide_count: z
        .number()
        .min(3)
    ...
    async ({
              ...
              text: JSON.stringify(
                {
                  ...
                  data: {
                    ...
                    templateId,
                    templatePackId: template_pack_id ?? null,
                    style: resolvedStyle,
                    ...
                    credits: {
                      ...
                      billingUnknownSlides: imageJobs.filter(job => job.billingStatus === 'unknown')
                        .length,
      ...
      const lines: string[] = [
        ...
        `  Template: ${templateId}${template_pack_id ? ` | Pack: ${template_pack_id}` : ''} | Style: ${resolvedStyle}`,

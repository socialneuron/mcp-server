  ...
  server.tool(
    ...
    "Pre-flight check before run_content_pipeline. Verifies: sufficient credits for estimated_posts, active OAuth on target platforms, brand profile exists, no stale insights. Returns pass/fail with specific issues to fix before running the pipeline.",
    {
      ...
      platforms: z
        .array(PLATFORM_ENUM)
        .min(1)
        .describe("Target platforms to check"),
      estimated_posts: z
        .number()
        .min(1)
        .max(50)
        .default(5)
        .describe("Estimated posts to generate"),
      response_format: z
        .enum(["text", "json"])
        .optional()
  ...
  server.tool(
    ...
    "Run the full content pipeline: research trends → generate plan → quality check → auto-approve → schedule posts. Chains all stages in one call for maximum efficiency. Set dry_run=true to preview the plan without publishing. To schedule posts, set schedule_confirmed=true after the user explicitly approves publishing. Check check_pipeline_readiness first to verify credits, OAuth, and brand profile are ready.",
    {
      ...
      topic: z
        .string()
        .optional()
        .describe("Content topic (required if no source_url)"),
      ...
      dry_run: z
        .boolean()
        .default(false)
        .describe("If true, skip scheduling and return plan only"),
    ...
    async ({
      ...
      try {
        ...
        const estimatedCost =
          BASE_PLAN_CREDITS + (source_url ? SOURCE_EXTRACTION_CREDITS : 0);
        ...
        const parsedPosts: ContentPlanPost[] = (postsArray ?? []).map(
          (p: any) => ({
        ...
        const invalidPlatformCount =
          parsedPosts.length - platformFilteredPosts.length;
        ...
        if (!skipSet.has("quality")) {
          for (const post of posts) {
            ...
            if (approval_mode === "auto" && quality.passed) {
              ...
              if (
                quality.total >= auto_approve_threshold &&
        ...
        if (!dry_run && !skipSet.has("schedule") && postsApproved > 0) {
          ...
          for (const post of approvedPosts) {
            ...
            const scheduledAt =
              post.schedule_at ??
              new Date(
                scheduleBaseMs + (Math.max(1, post.day) - 1) * 86_400_000,
              ).toISOString();
        ...
        if (response_format === "json") {
          return {
            content: [
              {
                type: "text" as const,
                text: JSON.stringify(asEnvelope(resultPayload), null, 2),
              },
            ],
          };
        }
        ...
        return {
          content: [
            { type: "text" as const, text: `Pipeline failed: ${message}` },
          ],
          isError: true,
        };
      }
  ...
  server.tool(
    ...
    "Check status of a pipeline run, including stages completed, pending approvals, and scheduled posts.",
    {
      pipeline_id: z
        ...
        .uuid()
        .optional()
        .describe("Pipeline run ID (omit for latest)"),
    ...
    async ({ pipeline_id, response_format }) => {
      ...
      if (
        Array.isArray(data.stages_skipped) &&
        data.stages_skipped.length > 0
      ) {
  ...
  server.tool(
    ...
    async ({ plan_id, project_id, dry_run, confirm, quality_threshold, response_format }) => {
      try {
        ...
        return {
          content: [
            { type: "text" as const, text: `Auto-approve failed: ${message}` },
          ],
          isError: true,
        };
      }

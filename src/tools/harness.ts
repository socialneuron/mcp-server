 ...
 *   - Provenance keys are restricted (quality-guard safety).
...
const writeReflectionSchema = {
  ...
  provenance: z
    ...
    .strict()
    .describe(
      'Source evidence for this reflection. Only these four keys are accepted (quality-guard guard — unknown keys are rejected to prevent spurious provenance claims).'
    ),
...
export function registerHarnessTools(server: McpServer, _ctx: unknown): void {
  ...
  server.tool(
    ...
    'Persist a verbal reflection for an agent loop. Provenance keys are restricted ' +
      '(quality-guard safety): only content_history_id, outcome_event_id, prm_score_ids, ' +

...
vi.mock("../lib/ssrf.js", async () => {
  const actual =
    await vi.importActual<typeof import("../lib/ssrf.js")>("../lib/ssrf.js");
  return {
    ...actual,
    validateUrlForSSRF: vi.fn(async (url: string) => {
      ...
      try {
        ...
        if (
          ...
          return {
            isValid: false,
            error: `Private/metadata IP: ${u.hostname}`,
          };
        }
...
describe("distribution tools", () => {
  ...
  describe("schedule_post", () => {
    ...
    it("normalizes platform names to capitalized convention", async () => {
      mockCallEdge.mockResolvedValueOnce(
        mockPreflightAccounts(["YouTube", "TikTok"]),
      );
      mockCallEdge.mockResolvedValueOnce({
        data: {
          success: true,
    ...
    it("maps snake_case params to camelCase in edge function body", async () => {
      mockCallEdge.mockResolvedValueOnce(
        mockPreflightAccounts(["Instagram"], "proj-123"),
      );
      mockCallEdge.mockResolvedValueOnce({
        data: {
          success: true,
    ...
    it("defaults platform AI disclosures on and preserves explicit false", async () => {
      mockCallEdge.mockResolvedValueOnce(
        mockPreflightAccounts(["TikTok", "Instagram", "YouTube"]),
      );
      mockCallEdge.mockResolvedValueOnce({
        data: {
          success: true,
    ...
    describe("auto_rehost", () => {
      it("rehosts external media_url via upload-to-r2 before posting", async () => {
        ...
        expect(mockCallEdge).toHaveBeenNthCalledWith(
          ...
          "upload-to-r2",
          expect.objectContaining({
            url: "https://replicate.delivery/xyz/ephemeral.png",
          }),
          expect.any(Object),
        );
        const scheduleCall = mockCallEdge.mock.calls.find(
          (c) => c[0] === "schedule-post",
        );
      ...
      it("does not trust an X-Amz-Signature query parameter as R2 provenance", async () => {
        ...
        expect(result.isError).toBe(false);
        expect(
          mockCallEdge.mock.calls.some((c) => c[0] === "upload-to-r2"),
        ).toBe(true);
        const scheduleCall = mockCallEdge.mock.calls.find(
          (c) => c[0] === "schedule-post",
        );
      ...
      it("rejects SSRF attempt disguised with X-Amz-Signature query param", async () => {
        ...
        const result = await handler({
          media_url:
            "http://169.254.169.254/latest/meta-data/?X-Amz-Signature=forged",
          caption: "hi",
          platforms: ["youtube"],
        });
      ...
      it("rehosts every URL in media_urls for carousels", async () => {
        ...
        mockCallEdge.mockResolvedValueOnce(
          mockPreflightAccounts(["Instagram"]),
        );
        mockCallEdge.mockResolvedValueOnce({
          data: {
            success: true,
        ...
        const uploads = mockCallEdge.mock.calls.filter(
          (c) => c[0] === "upload-to-r2",
        );
      ...
      it("rehosts a raw (non-R2) result_url returned by a kie.ai job_id", async () => {
        ...
        expect(uploadCall[1]).toEqual(
          expect.objectContaining({
            url: "https://tempfile.kie.ai/abc/generated.mp4",
          }),
        );
        const scheduleCall = mockCallEdge.mock.calls.find(
          (c) => c[0] === "schedule-post",
      ...
      it("does not rehost a kie.ai job_id that already resolved to an R2 key", async () => {
        ...
        mockCallEdge
          ...
          .mockResolvedValueOnce({
            data: {
              signedUrl: "https://r2.example.com/k.mp4?X-Amz-Signature=sig",
            },
            error: null,
          })
          .mockResolvedValueOnce(mockPreflightAccounts(["YouTube"]))
      ...
      it("does not rehost when r2_key is already provided", async () => {
        mockCallEdge
          .mockResolvedValueOnce({
            data: {
              signedUrl: "https://r2.example.com/object?X-Amz-Signature=sig",
            },
            error: null,
          })
          .mockResolvedValueOnce(mockPreflightAccounts(["YouTube"]))
  ...
  describe("schedule_content_plan", () => {
    ...
    function mockPlanEdgeCalls(
      ...
      mockCallEdge.mockImplementation(async (fn: string, body: any) => {
        ...
        if (fn === "mcp-data" && body.action === "list-plan-approvals") {
          return (
            approvalsData ?? { data: { success: true, items: [] }, error: null }
          );
        }
        ...
        if (fn === "schedule-post") {
          return {
            data: {
              success: true,
              results: {
                Twitter: { success: true, postId: "post-1", jobId: "job-1" },
              },
    ...
    it("filters to approved/edited posts when plan approvals exist", async () => {
      ...
      const posts = [
        {
          id: "day1-twitter-1",
          caption:
            "Approved post caption with enough content to pass baseline checks.",
          platform: "twitter",
        ...
        },
        {
          id: "day1-linkedin-1",
          caption:
            "Original edited caption that should be replaced by edited_post.",
          platform: "linkedin",
      ...
      mockPlanEdgeCalls(
        {
          data: {
            success: true,
            plan: { id: planId, plan_payload: { posts } },
          },
          error: null,
        },
        {
      ...
      const returnedPosts = envelope.data.posts as Array<{
        id: string;
        caption: string;
      }>;
      ...
      expect(returnedPosts.map((p) => p.caption)).toContain(
        "Edited approved caption",
      );
      // Verify schedule-post was NOT called (dry_run)
      const schedulePostCalls = mockCallEdge.mock.calls.filter(
        (c) => c[0] === "schedule-post",
      );
              ...
              posts: [
                {
                  id: "day1-twitter-1",
                  caption:
                    "High quality caption with clear CTA for audience growth.",
                  platform: "twitter",
                  schedule_at: "2026-03-20T10:00:00Z",
                },
              ],

...
function run(
  args: string[],
  opts?: { stripAuth?: boolean }
): { stdout: string; stderr: string; exitCode: number } {
  const env: Record<string, string | undefined> = { ...process.env, NO_COLOR: '1' };
  if (opts?.stripAuth) {
    // Remove all auth sources so initializeAuth() fails fast without network calls.
    // Set a dummy API key so loadApiKey() returns immediately (skips macOS Keychain).
    // Point Supabase URL to localhost so validateApiKey() fails fast (connection refused).
    delete env.SOCIALNEURON_SERVICE_KEY;
    delete env.SUPABASE_SERVICE_ROLE_KEY;
    env.SOCIALNEURON_API_KEY = 'snk_test_invalid_000000';
...
// All tests require the built binary — skip gracefully in CI without build step
const describeE2E = hasBinary ? describe : describe.skip;

// ---------------------------------------------------------------------------
// Top-level commands
// ---------------------------------------------------------------------------

describeE2E('top-level commands', () => {
...
describeE2E('unified JSON envelope', () => {
  it('all JSON outputs include schema_version "1"', { timeout: 15_000 }, () => {
    const cases = [
      ['--version', '--json'],
      ['--help', '--json'],
      ['sn', 'tools', '--json'],
...
describeE2E('sn plan (auth error)', () => {
  it('list without auth returns AUTH error with hint', () => {
    const { stdout, exitCode } = run(['sn', 'plan', 'list', '--json'], { stripAuth: true });
    expect(exitCode).toBe(1);
    const json = parseJson(stdout);
    expect(json.ok).toBe(false);

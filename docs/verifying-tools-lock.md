...
`@socialneuron/mcp-server` ships a sealed manifest, [`tools.lock.json`](../tools.lock.json), containing a SHA-256 hash of every tool's model-visible metadata. Pin a hash in your agent's configuration and verify at runtime to detect rug-pull attacks.
...
1. Instantiates the server and runs `registerAllTools(server, { skipApps: true })`, then enumerates the registered runtime tools shipped in the npm package.
...
3. Enumerates `src/lib/tool-catalog.ts`, the static catalog served by `search_tools`. This adds HTTP-only catalog entries such as `open_content_calendar`, so the lock covers both runtime and catalog-visible surfaces.
...
- You see a PR in the public repo that changes tool runtime metadata (`src/tools/*.ts` descriptions, schema descriptions, annotations, `_meta`, or scopes) or `src/lib/tool-catalog.ts`

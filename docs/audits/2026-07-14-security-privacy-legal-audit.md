...
| High confidentiality | Tier-locked living skills could return their full paid guide and compiled performance section | The list correctly marked rows locked, but the detail path treated the lock as an annotation and returned the body                   | Candidate fails closed in the MCP projection and redacts locked body fields in `mcp-data`; deploy and test with a lower-tier account before release                                         |
...
| Medium operational   | Claude/Codex calls exhausted the 10-session per-user cap                                      | Idle client sessions produced a 30-minute self-denial even though no generation or publishing rate limit was reached                 | Candidate reclaims only the oldest idle user/global session and protects active requests/SSE; deploy and prove more than 10 sequential sessions before release                              |
...
- Model-fallback reasons and skill-catalogue failures are treated as untrusted upstream text and reduced to stable public wording before they reach a client.
...
- Living-skill bodies and compiled performance sections are plan-gated intellectual property. A locked catalogue row may expose discovery metadata, but neither the backend nor any MCP/REST projection may return its body.
...
- private connector PR `#2223` is independently approved, merged, deployed, and passes Claude/Codex discovery plus a greater-than-10-session live regression;
- GitHub account billing/spending limits allow required CI jobs to start and pass;

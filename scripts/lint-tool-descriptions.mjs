#!/usr/bin/env node
/**
 * Lint runtime MCP tool metadata for prompt-injection patterns.
 *
 * Checks the model-visible strings emitted by the real stdio runtime registry:
 * tool titles/descriptions, input/output schema titles/descriptions,
 * annotations, and _meta strings. This keeps the lint aligned with the
 * tools/list surface protected by tools.lock.json.
 *
...
import { collectModelVisibleText, enumerateRuntimeTools } from './lib/enumerate-runtime-tools.mjs';

// Invisible / formatting characters that have no place in tool metadata.
const ZERO_WIDTH = /[\u200B-\u200F\u202A-\u202E\u2060-\u206F\uFEFF]/;
...
// Email in tool metadata is almost always an exfil target.
...
const URL_ALLOWLIST = new Set([
  ...
  'socialneuron.com',
  'github.com',
  // RFC 2606 reserved domain used only in parameter examples.
  'example.com',
]);
...
const textTargets = Object.entries(runtime).flatMap(([name, info]) => collectModelVisibleText(name, info));

let totalFindings = 0;
const perTarget = [];
for (const target of textTargets) {
  const findings = lintDescription(target.text);
  if (findings.length) {
    perTarget.push({ ...target, findings });
...
if (totalFindings === 0) {
  console.log(
    `✅ Lint passed: ${textTargets.length} model-visible strings across ${Object.keys(runtime).length} runtime tools are clean.`
  );
  process.exit(0);
}

console.error('❌ Runtime tool metadata lint failed:\n');
for (const { name, path, findings } of perTarget) {
  console.error(`  ${name} (${path}):`);
  for (const f of findings) console.error(`    - ${f}`);
}
console.error(
  `\n${totalFindings} finding(s) across ${perTarget.length} model-visible string(s), out of ${textTargets.length} checked.`
);

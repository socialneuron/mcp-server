...
- Lifecycle deletion/cancellation and failed-job billing are not considered production-verified until the compatible private backend is deployed and the evidence matrix in [`an internal test plan`](an internal test plan) passes against the hosted authenticated service.
